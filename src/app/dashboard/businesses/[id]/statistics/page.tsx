...
async function getBusinessStats(id: string, userId: string) {
  const business = await prisma.business.findUnique({
    where: { id },
  })

  if (!business) return null
  if (business.ownerId !== userId && userId !== "admin") return null
...
export default async function BusinessStatisticsPage({
  ...
  const data = await getBusinessStats(params.id, user.id)
  if (!data) notFound()
